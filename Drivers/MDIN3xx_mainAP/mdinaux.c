...
static void MDINAUX_Get4CHScale(PMDIN_VIDEO_INFO pINFO)
{
	...
	switch (p4CH->view) {
		...
		case MDIN_4CHVIEW_CH12H:	case MDIN_4CHVIEW_CH13H: 
		case MDIN_4CHVIEW_CH14H: case MDIN_4CHVIEW_CH23H: 
		case MDIN_4CHVIEW_CH24H:	case MDIN_4CHVIEW_CH34H:
				 pMFC->st4CH.w = pMFC->stDST.w/2; pMFC->st4CH.h = pMFC->stDST.h/1; break;	// 2ch multi-channel (horizontal direction)
 		case MDIN_4CHVIEW_CH12V:	case MDIN_4CHVIEW_CH13V:
		case MDIN_4CHVIEW_CH14V: case MDIN_4CHVIEW_CH23V:
		case MDIN_4CHVIEW_CH24V:	case MDIN_4CHVIEW_CH34V:
				pMFC->st4CH.w = pMFC->stDST.w/1; pMFC->st4CH.h = pMFC->stDST.h/2; break; // 2ch multi-channel (veritical direction)
...
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x141, 1, 1, mode)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x141, 1, 1, 1)) return MDIN_I2C_ERROR;	// enable ext-buff mode (for multi-window mode)
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x141, 1, 1, 0)) return MDIN_I2C_ERROR;	// disable ext-buff mode (for multi-channel mode)

	// if src is progressive && out is interlace && V dn-scaler then aux_rd_frm2fld = 1
	// if src is interlace && out is interlace && input V-size greater than 2 times output V-size then aux_rd_frm2fld = 1  // added on 15May2014
...
//	if ((pSRC->stATTB.attb&MDIN_SCANTYPE_PROG)&&(pMFC->stCUT.h>(pMFC->stDST.h*2))&& 	// modified on 29Mar2012
	if ((pSRC->stATTB.attb&MDIN_SCANTYPE_PROG)&&(pMFC->stCUT.h>=(pMFC->stDST.h*2))&& 	// modified on 30Dec2015
		(pOUT->stATTB.attb&MDIN_SCANTYPE_PROG)==0)	mode = 1;
	else if ((pSRC->stATTB.attb&MDIN_SCANTYPE_PROG)==0&&(pMFC->stCUT.h>(pMFC->stDST.h*2))&& // added on 15May2014
		(pOUT->stATTB.attb&MDIN_SCANTYPE_PROG)==0)	mode = 1;
	else											mode = 0;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x142, 13, 1, mode)) return MDIN_I2C_ERROR;

	// if src is interlace && out is interlace && V dn-scaler then aux_skip_dis = 1
	if ((pSRC->stATTB.attb&MDIN_SCANTYPE_PROG)==0&&(pMFC->stCUT.h>pMFC->stDST.h)&&
		(pOUT->stATTB.attb&MDIN_SCANTYPE_PROG)==0)	mode = 1;
	else											mode = 0;
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x144, 5, 1, mode)) return MDIN_I2C_ERROR;		// deleted on 15May2014
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x144, 7, 1, mode)) return MDIN_I2C_ERROR;	// modified on 15May2014
...
#if __MDIN3xx_DBGPRT__ == 1
	UARTprintf("pMFC->stCUT.h=%d, pMFC->stSRC.h=%d\n", pINFO->stMFC_x.stCUT.h, pINFO->stMFC_x.stSRC.h);
	UARTprintf("pMFC->stDST.h=%d, pMFC->stCRS.dh=%d\n", pINFO->stMFC_x.stDST.h, pINFO->stMFC_x.stCRS.dh);
	UARTprintf("ratio = %d\n", ratio);
#endif
...
#if	defined(SYSTEM_USE_MDIN325)||defined(SYSTEM_USE_MDIN325A)||defined(SYSTEM_USE_MDIN380)
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDINAUX_SetEncTestPattern(PMDIN_VIDEO_INFO pINFO, MDIN_ENC_TEST_t mode)
{
	BYTE nID;

	if (pINFO->encPATH==VENC_PATH_PORT_A) {
		switch (pINFO->stSRC_a.frmt) {
			case VIDSRC_720x576i50: pINFO->encFRMT = VID_VENC_PAL_B;  break;
			default:				pINFO->encFRMT = VID_VENC_NTSC_M; break;
		}
	}
	else if (pINFO->encPATH==VENC_PATH_PORT_B) {
		switch (pINFO->stSRC_b.frmt) {
			case VIDSRC_720x576i50: pINFO->encFRMT = VID_VENC_PAL_B;  break;
			default:				pINFO->encFRMT = VID_VENC_NTSC_M; break;
		}
	}
	else { // pINFO->encPATH==VENC_PATH_PORT_X
		switch (pINFO->stOUT_x.frmt) {
			case VIDOUT_720x576i50: pINFO->encFRMT = VID_VENC_PAL_B;  break;
			default:				pINFO->encFRMT = VID_VENC_NTSC_M; break;
		}
	...
	switch (pINFO->encFRMT) {
		case VID_VENC_NTSC_M:	nID = 0; break;
		case VID_VENC_NTSC_J:	nID = 0; break;
		case VID_VENC_NTSC_443:	nID = 0; break;
		case VID_VENC_PAL_B:	nID = 1; break;
		case VID_VENC_PAL_D:	nID = 1; break;
		case VID_VENC_PAL_G:	nID = 1; break;
		case VID_VENC_PAL_H:	nID = 1; break;
		case VID_VENC_PAL_I:	nID = 1; break;
		case VID_VENC_PAL_M:	nID = 0; break;
		case VID_VENC_PAL_N:	nID = 1; break;
		case VID_VENC_PAL_Nc:	nID = 1; break;
		default:					nID = 0; break;
	}

	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x1e0, 3, 1, nID)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x1e0, 0, 3, mode)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
#endif
