...
static MDIN_ERROR_t MDINHTX_SuspendRiCheck(BOOL OnOff)
{
	...
	if (OnOff==ON) {
		...
		if (MDINHIF_RegRead(MDIN_HDMI_ID, 0x026, &GetMODE)) return MDIN_I2C_ERROR; // move here to check MDDC bus status for Ri128 (20150401)
		GetMODE = LOBYTE(GetMODE)&0x01; // Save MDDC bus status						 // move here to check MDDC bus status for Ri128 (20150401)
...
static MDIN_ERROR_t MDINHTX_SetHDCPAuthMode(PMDIN_HDMICTRL_INFO pCTL)
{
	...
	MDINDLY_mSec(40);	// Delay for auto Ri status set (20150401)
...
//--------------------------------------------------------------------------------------------------------------------------
static MDIN_ERROR_t MDINHTX_SetSoftNVAL(PMDIN_VIDEO_INFO pINFO, PMDIN_HTXAUDIO_INFO pAUD)
{
	...
	switch (LO4BIT(pAUD->freq)) {
		case AUDIO_FREQ_32kHz:	
			if(pINFO->stOUT_m.frmt==VIDOUT_1280x720p59 )		nVal =  11648; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080i59 )	nVal =  11648; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080p59 )	nVal =  11648; 
			else												nVal =  4096;
			break;
		case AUDIO_FREQ_44kHz:
			if(pINFO->stOUT_m.frmt==VIDOUT_1280x720p59 )		nVal =	17836; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080i59 ) 	nVal =	17836; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080p59 ) 	nVal =	8918; 
			else												nVal =	6272;
			break;
		case AUDIO_FREQ_48kHz:
			if(pINFO->stOUT_m.frmt==VIDOUT_1280x720p59 )		nVal =	11648; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080i59 ) 	nVal =	11648; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080p59 ) 	nVal =	5824; 
			else												nVal =	6144;
			break;
		case AUDIO_FREQ_88kHz:
			if(pINFO->stOUT_m.frmt==VIDOUT_1280x720p59 )		nVal =	35672; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080i59 ) 	nVal =	35672; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080p59 ) 	nVal =	17836; 
			else												nVal =	12544;
			break;
		case AUDIO_FREQ_96kHz:
			if(pINFO->stOUT_m.frmt==VIDOUT_1280x720p59 )		nVal =	23296; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080i59 ) 	nVal =	23296; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080p59 ) 	nVal =	11648; 
			else												nVal =	12288;
			break;
		case AUDIO_FREQ_176kHz:
			if(pINFO->stOUT_m.frmt==VIDOUT_1280x720p59 )		nVal =	71344; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080i59 ) 	nVal =	71344; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080p59 ) 	nVal =	35672; 
			else												nVal =	25088;
			break;
		case AUDIO_FREQ_192kHz:
			if(pINFO->stOUT_m.frmt==VIDOUT_1280x720p59 )		nVal =	46592; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080i59 ) 	nVal =	46592; 
			else if(pINFO->stOUT_m.frmt==VIDOUT_1920x1080p59 ) 	nVal =	23296; 
			else												nVal =	24576;
			break;
...
static MDIN_ERROR_t MDINHTX_SetAudioInit(PMDIN_VIDEO_INFO pINFO)
{
	...
	// set software N_VAL
	if (MDINHTX_SetSoftNVAL(pINFO, pAUD)) return MDIN_I2C_ERROR;
...
static MDIN_ERROR_t MDINHTX_HPDHandler(PMDIN_HDMICTRL_INFO pCTL)
{
	...
	if (rVal&0x0200) {
		...
		GetPLUG = 0; GetEDID = 0; pCTL->proc = HTX_CABLE_PLUG_OUT;
//		CECDisconnect();
	}
