...
static MDIN_ERROR_t MDIN3xx_SetSrcVideoPort(PMDIN_VIDEO_INFO pINFO, WORD nID)
{
	...
	switch (pSRC->frmt) {
		...
		case VIDSRC_1920x1080p60:	case VIDSRC_1920x1080p50:	case VIDSRC_1920x1080p24:
			pSRC->stATTB.attb |= (MDIN_QUALITY_HD|MDIN_VIDEO_SYSTEM); break;
		default:
...
static MDIN_ERROR_t MDIN3xx_SetOutVideoFrmt(PMDIN_VIDEO_INFO pINFO)
{
	...
	switch (pOUT->frmt) {
		...
		case VIDOUT_1920x1080p30:	case VIDOUT_1920x1080p29: case VIDOUT_1920x1080p25:	case VIDOUT_1920x1080p24:
			pOUT->stATTB.attb |= (MDIN_WIDE_RATIO|MDIN_QUALITY_HD|MDIN_VIDEO_SYSTEM); break;
	...
	switch (pOUT->mode) {
		case MDIN_OUT_RGB444_8:		mode = (1<<12)|(2<<8)|(0<<5)|(1<<2)|0;
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_444|MDIN_COLORSPACE_RGB); break;
		case MDIN_OUT_YUV444_8:		mode = (1<<12)|(0<<8)|(2<<5)|(1<<2)|0;
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_444|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_EMB422_8:		mode = (1<<12)|(0<<8)|(3<<5)|(3<<2)|1;
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_SEP422_8:		mode = (1<<12)|(0<<8)|(2<<5)|(1<<2)|1;
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_EMB422_10:	mode = (0<<12)|(0<<8)|(3<<5)|(2<<2)|1;
			pOUT->stATTB.attb |= (MDIN_PRECISION_10|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_SEP422_10:	mode = (0<<12)|(0<<8)|(2<<5)|(0<<2)|1;
			pOUT->stATTB.attb |= (MDIN_PRECISION_10|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_MUX656_8:		mode = (1<<12)|(0<<8)|(3<<5)|(3<<2)|2;
			pOUT->stATTB.attb |= (MDIN_PRECISION_8|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
		case MDIN_OUT_MUX656_10:	mode = (0<<12)|(0<<8)|(3<<5)|(2<<2)|2;
			pOUT->stATTB.attb |= (MDIN_PRECISION_10|MDIN_PIXELSIZE_422|MDIN_COLORSPACE_YUV); break;
	}
...
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x0a2, 0, 10, mode)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x0a2, 0, 13, mode)) return MDIN_I2C_ERROR;	// add dither on/off (30Dec2015)
...
static MDIN_ERROR_t MDIN3xx_SetOutVideoSYNC(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (pINFO->dacPATH==DAC_PATH_AUX_4CH) {		// for 4-CH input mode
		...
		if (pAUX->frmt==VIDOUT_1920x1080p60||pAUX->frmt==VIDOUT_1920x1080p59||
		    pAUX->frmt==VIDOUT_1920x1080p30||pAUX->frmt==VIDOUT_1920x1080p29) stSYNC.totHS = 1082;
...
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x083, 6, 1, 1)) return MDIN_I2C_ERROR;	// fix main_skip_dis // deleted on 16Aug2012
	...
	// sync_ctrl
	mode = (1<<15)|(10<<11)|(1<<9)|(1<<2)|MBIT(pOUT->fine,MDIN_SYNC_FREERUN);	// sync_reset_dly_en=1, sync_reset_on_field=1 on 13Apr2013
...
static MDIN_ERROR_t MDIN3xx_SetMFCScaleWind(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x031, pMFC->stSRC.y)) return MDIN_I2C_ERROR;	// 12Apr2013
...
static MDIN_ERROR_t MDIN3xx_SetMFCScaleCtrl(PMDIN_VIDEO_INFO pINFO)
{
	...
	if ((pOUT->stATTB.attb&MDIN_SCANTYPE_PROG)==0) 
			mode = (pMFC->stFFC.sh>=pMFC->stDST.h*4)? (3<<2)|3 : (2<<2)|2;
	else 	mode = (pMFC->stFFC.sh>=pMFC->stDST.h*2)? (3<<2)|3 : (2<<2)|2;
//	UARTprintf("pMFC->stFFC.sh = %d, pMFC->stDST.h = %d\n", pMFC->stFFC.sh, pMFC->stDST.h);
...
//	mode = ((pMFC->stFFC.dh>pMFC->stMEM.h)&&((mode&3)==3))? 1 : 0;
	mode = ((pMFC->stFFC.dh>pMFC->stMEM.h)&&(pMFC->stSRC.h>pMFC->stDST.h*2))? 1 : 0;
#if __MDIN3xx_DBGPRT__ == 1
	UARTprintf("pMFC->stFFC.dh=%d, pMFC->stMEM.h=%d\n", pMFC->stFFC.dh, pMFC->stMEM.h);
	UARTprintf("pMFC->stSRC.h=%d, pMFC->stDST.h=%d\n", pMFC->stSRC.h, pMFC->stDST.h);
#endif	
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x100, 1, 1, mode)) return MDIN_I2C_ERROR; // mfc_itp_linear
...
//	PMDIN_SRCVIDEO_INFO pSRC = (PMDIN_SRCVIDEO_INFO)&pINFO->stSRC_m;
	...
	else if (pMFC->stFFC.dw < 480)						nID = 4;	// added on 12Apr2013
	else										nID = 0;
...
//	if (pMFC->stSRC.h>=pMFC->stDST.h*2)
	...
	if (pSRC->frmt==VIDSRC_1920x1080p60||pSRC->frmt==VIDSRC_1920x1080p50||pSRC->frmt==VIDSRC_1920x1080p24)
	...
	// set frc down flag, if output is 720p30,25,1080p30,25	// 05Apr2012, modified on 17Oct2013
	if (pOUT->frmt==VIDOUT_1280x720p30||pOUT->frmt==VIDOUT_1280x720p25||pOUT->frmt==VIDOUT_1280x720p24||
		pOUT->frmt==VIDOUT_1920x1080p30||pOUT->frmt==VIDOUT_1920x1080p29||
		pOUT->frmt==VIDOUT_1920x1080p25||pOUT->frmt==VIDOUT_1920x1080p24)
...
//	}
	...
	// modified on 17Oct2013
	if (pIPC->attb&MDIN_DEINT_IPC_PROC)	{ // if interlace input
		if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x083,  6, 1, 1)) return MDIN_I2C_ERROR;	// fix main_skip_dis
	}
	else {			// if progressive input
		if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x083,  6, 1, (pIPC->fine&MDIN_DEINT_FRC_DOWN)? 0 : 1)) return MDIN_I2C_ERROR; // main_skip_dis, added on 28Dec2012
	}
...
static MDIN_ERROR_t MDIN3xx_Set4CHProcess(PMDIN_VIDEO_INFO pINFO)
{
	...
	switch (pINFO->st4CH_x.view) {
		...
		case MDIN_4CHVIEW_CH12H: case MDIN_4CHVIEW_CH12V:	rVal = nA1|(nA2<<2)|(nB1<<4)|(nB2<<6); break;
		case MDIN_4CHVIEW_CH13H: case MDIN_4CHVIEW_CH13V:	rVal = nA1|(nB1<<2)|(nB1<<4)|(nB2<<6); break;
		case MDIN_4CHVIEW_CH14H: case MDIN_4CHVIEW_CH14V:	rVal = nA1|(nB2<<2)|(nB1<<4)|(nB2<<6); break;
		case MDIN_4CHVIEW_CH23H: case MDIN_4CHVIEW_CH23V:	rVal = nA2|(nB1<<2)|(nB1<<4)|(nB2<<6); break;
		case MDIN_4CHVIEW_CH24H: case MDIN_4CHVIEW_CH24V:	rVal = nA2|(nB2<<2)|(nB1<<4)|(nB2<<6); break;
		case MDIN_4CHVIEW_CH34H: case MDIN_4CHVIEW_CH34V:	rVal = nB1|(nB2<<2)|(nB1<<4)|(nB2<<6); break;
		default:				rVal = nA1|(nA2<<2)|(nB1<<4)|(nB2<<6); break;
	}
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x14a, 0, 8, rVal)) return MDIN_I2C_ERROR;
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_SetSrcHACTPolarity(PMDIN_VIDEO_INFO pINFO, BOOL edge)	// control input hsync polarity (POSI=1, NEGA=0)
{
	PMDIN_SRCVIDEO_INFO pSRC = (PMDIN_SRCVIDEO_INFO)&pINFO->stSRC_m;
	WORD nID = (pSRC->stATTB.attb&MDIN_USE_INPORT_A)? 1 : 0;
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_SetSrcVACTPolarity(PMDIN_VIDEO_INFO pINFO, BOOL edge)	// control input vsync polarity (POSI=1, NEGA=0)
{
	PMDIN_SRCVIDEO_INFO pSRC = (PMDIN_SRCVIDEO_INFO)&pINFO->stSRC_m;
	WORD nID = (pSRC->stATTB.attb&MDIN_USE_INPORT_A)? 1 : 0;
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_SetDelayVCLK(MDIN_CLK_DELAY_t delay)	// added on 22Oct2013
{
	if (MDINHIF_RegField(MDIN_HOST_ID, 0x018, 0, 3, delay)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
//	if (MDIN3xx_SetMemoryPLL((GetMEM)? 1 : 4, (GetMEM)? 12 : 59, 0)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(MDIN_HOST_ID, 0x090, 1, 1, 1)) return MDIN_I2C_ERROR;	// dfi iusrrst
...
//	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x1a3, 0x1867)) return MDIN_I2C_ERROR;	// ddr timing tune
	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x1a7, 0x4008)) return MDIN_I2C_ERROR;	// ddr initialize, bit14=on the rising edge of this bit, ddr is initialized
	if (MDINHIF_RegWrite(MDIN_HOST_ID,  0x093, 0x2f80)) return MDIN_I2C_ERROR;	// dfi latency init
	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x1a7, 0x2008)) return MDIN_I2C_ERROR;	// ddr access enable, bit13=ddr access enable
	...
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x1a7, 12, 1,  GetMEM&1)) return MDIN_I2C_ERROR;	// column address bit width of ddr ('0':8 bits, '1':7 bits)
	...
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x1a3, 11, 2, (GetMEM)? 0 : 3)) return MDIN_I2C_ERROR;		// ddr timing tune
...
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x0a1, 8, 4, Offset_S)) return MDIN_I2C_ERROR;	// vid_act_start_offset
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x0a1, 4, 4, Offset_E)) return MDIN_I2C_ERROR;	// vid_act_end_offset
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x0a1, 0, 1, OnOff)) return MDIN_I2C_ERROR;		// vid_act_on_de
	return MDIN_NO_ERROR;
}
...
//	BuffSize = ROUNDUP(pMFC->stFFC.dw, UnitSize)*UnitSize;
	...
	MDIN3xx_SetMemoryReAlloc(pINFO);
...
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x06c, 2, 3, blendlevel)) return MDIN_I2C_ERROR; // PIP blend mode (0:video video opaque, 1:1/4, 2:1/2, 3:3/4, 4:main video opaque)
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x06c, 6, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;	// PIP chroma key enable
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x06d, 0, 8, l_bound)) return MDIN_I2C_ERROR;	// lower bound of chroma key range (Cb & Cr)
	return MDIN_NO_ERROR;	
}
...
/*******************************************************************************************/
/* Parameter : src - CLK_A or CLK_B */
/* OnOff ? ON or OFF */
/*******************************************************************************************/
MDIN_ERROR_t MDIN3xx_ExtSyncClkSel(MDIN_PLL_SOURCE_t src, BOOL OnOff)
{
	if (MDINHIF_RegField(MDIN_HOST_ID, 0x042, 4, 1, OnOff)) return MDIN_I2C_ERROR;
	return MDIN3xx_SetVCLKPLLSource(src);
}

/*******************************************************************************************/
/* Parameter : ExtSyncSrc = '000'(port A), '001'(port B) */
/*******************************************************************************************/
MDIN_ERROR_t MDIN3xx_ExtSyncSyncSel(BYTE ExtSyncSrc, BOOL OnOff)
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x098, 3, 3, ExtSyncSrc)) return MDIN_I2C_ERROR;
	// 1:lock to another sync src
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x098, 8, 1, OnOff)) return MDIN_I2C_ERROR;  // lock_to_ext_sync
	return MDIN_NO_ERROR;
}

/*******************************************************************************************/
/* Parameter : OnOff - 0:frame-lock mode, 1:free run mode */
/*******************************************************************************************/
MDIN_ERROR_t MDIN3xx_ExtSyncLockEn(BOOL OnOff)
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x098, 0, 1, RBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}

/*******************************************************************************************/
/* Parameter : delay ? line delay number */
/*******************************************************************************************/
...
/*******************************************************************************************/
/* Parameter : delay ? pixel delay number */
/*******************************************************************************************/
MDIN_ERROR_t MDIN3xx_ExtSyncHSDelay (WORD delay)
{
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x097, 4, 12, delay)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x099, 15, 1, 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x099, 15, 1, 1)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	MDIN3xx_ExtSyncClkSel(MDIN_PLL_SOURCE_CLKA, OnOff);	//  CLK_A is external clock
	MDIN3xx_ExtSyncSyncSel(0, OnOff);	//  ¡®001¡¯ : B Port
...
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_SetLineDeci(BYTE dfactor)    // dfactor is v-decimation factor : 1/dfactor (1 is off)
{
    WORD rVal;
    
    if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x01d, 11, 5, dfactor-1)) return MDIN_I2C_ERROR;
    if (MDINHIF_RegRead(MDIN_LOCAL_ID, 0x109, &rVal)) return MDIN_I2C_ERROR;
    if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x109, rVal/dfactor)) return MDIN_I2C_ERROR;
    return MDIN_NO_ERROR;
}

//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_EnableDigitalBypass(BOOL OnOff, BOOL MODE444, BYTE nID) // nID: 1 (port A), 0 (port B)
{
	MDIN3xx_ExtSyncClkSel(((nID)? MDIN_PLL_SOURCE_CLKA:MDIN_PLL_SOURCE_CLKB), OnOff); // input clock bypass
	MDINI2C_RegField(MDIN_LOCAL_ID, 0x06c, 7, 1, OnOff); 		// input bypass to output block
	MDINI2C_RegField(MDIN_LOCAL_ID, 0x06c, 1, 1, MODE444);	// 1 for 444, 0 for 422
	MDIN3xx_ExtSyncHSDelay(45);	// adjust H position of output
	MDIN3xx_ExtSyncVSDelay(4);		// adjust V position of output
	return MDIN_NO_ERROR;
}

//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_EnableDACBypass(BOOL OnOff, MDIN_CLK_DELAY_t delay)	// available for rgb444/yuv444 & sync separated input
{
	MDIN3xx_SetDelayVCLK(delay);
	MDIN3xx_ExtSyncClkSel(MDIN_PLL_SOURCE_CLKA, OnOff); //  CLK_A is external clock
	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x0a5, 11, 1, OnOff)) return MDIN_I2C_ERROR; // digital data, h/vsync inputs are directly fed to DAC
	return MDIN_NO_ERROR;
}

#if	defined(SYSTEM_USE_MDIN340)||defined(SYSTEM_USE_MDIN380)
//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_EnableHDMIBypass(BOOL OnOff, PMDIN_VIDEO_INFO pINFO, BYTE nID) // nID: 1 (port A), 0 (port B)
{
	PMDIN_SRCVIDEO_INFO pSRC = (PMDIN_SRCVIDEO_INFO)((nID==1)? &pINFO->stSRC_a : &pINFO->stSRC_b);
	WORD mode = 0, acen = 0;
	
	// input clock bypass
	MDIN3xx_ExtSyncClkSel(((nID)? MDIN_PLL_SOURCE_CLKA:MDIN_PLL_SOURCE_CLKB), OnOff); // CLK_B is external clock
	
	// input data bypass
//	MDINI2C_RegField(MDIN_LOCAL_ID, 0x0a5, 8, 2, 0); // hdmi_src_sel
	MDINI2C_RegField(MDIN_LOCAL_ID, 0x0a5, 6, 1, OnOff); // input_bypass_to_hdmi
	MDINI2C_RegField(MDIN_LOCAL_ID, 0x0a5, 5, 1, 0); // hdmi_data_mode (0: 36bit, 1: 24bit)
	MDINI2C_RegField(MDIN_HDMI_ID, 0x048, 14, 2, 2); // number of bits (when use hdmi bypass mode, it should be fixed as '2')

	switch (pSRC->mode) {
		case MDIN_SRC_RGB444_8 :
		...
		case MDIN_SRC_MUX656_8 :
			...
			break;
		case MDIN_SRC_SEP422_8: case MDIN_SRC_SEP422_10:
			switch (pINFO->stVID_h.mode) {
			...
			break;
		case MDIN_SRC_EMB422_8: case MDIN_SRC_EMB422_10:
			switch (pINFO->stVID_h.mode) {
				case HDMI_OUT_RGB444_8:	mode = 0x3d; acen = 0x00; break;	// dither,YUV2RGB,444 / bypass
				case HDMI_OUT_YUV444_8:	mode = 0x05; acen = 0x00; break;	// 444 / bypass
				case HDMI_OUT_SEP422_8: mode = 0x21; acen = 0x00; break;	// dither / bypass
			}
			break;
	}
